...
const BookingForm = () => {
    ...
    const fetchBookingStatus = async (selectedDate) => {
        try {
            const response = await fetch(`https://taaza-dandiya-backend.onrender.com/api/admin/booking-status/${selectedDate}`);
    ...
    const verifyDiscountCode = async () => {
        if (formData.discountCode) {
            try {
                ...
                const response = await fetch('https://taaza-dandiya-backend.onrender.com/api/admin/verify-coupon', {
                    method: 'POST',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const phoneCheckRes = await axios.get(`https://taaza-dandiya-backend.onrender.com/api/bookings/check-phone/${formData.phone}`);
            ...
            const limitCheckRes = await axios.get(`https://taaza-dandiya-backend.onrender.com/api/bookings/check-limit/${formData.date}/${formData.tickets}`);
    ...
    return (
        <form onSubmit={handleSubmit} className='inter booking-form flex flex-col gap-3 shadow-xl border-2 border-slate-300 bg-white p-6'>
            ...
            <p className='font-light font-xs leading-3'>1. You can book a minimum of 1 and a maximum of 5 tickets from one mobile number.</p>
            <p className='font-light font-xs leading-3'>2. Online Tickets will have to be changed for physical tickets at the ticket counter at venue.</p>
            <p className='font-light font-xs leading-3'>3. Only 1 booking allowed per phone number </p>
            <p className='font-light font-xs leading-3'>4. Only successfully paid tickets will be accepted for entry.</p>
            ...
            <p className='font-light font-xs leading-3'>16. Organisers hold the rights to deny late entry.</p>
        </form>
    );
};
